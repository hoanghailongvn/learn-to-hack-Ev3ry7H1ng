...
def simulate():
    ...
    # Attacker -> Server: I, A = 0
    I_A = a.send_I_A()
    s.recv_I_A(I_A)

    # Server -> Attacker: salt, B
    salt_B = s.send_salt_B()
    a.recv_salt_B(salt_B)

    # Attacker -> Server: hmac: được tính với key = 0 và salt lấy từ server
    hmac_value = a.send_hmac()
    result = s.recv_hmac(hmac_value)
    a.recv_result(result)
